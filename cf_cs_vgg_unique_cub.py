...
def main():
    ...
    topK_prob_predicted_classes, _ = smallest_indices_each_example(all_class_dis_teacher, 10)
    ...
    recall, precision = cf_proposal_extraction(val_hard_loader, heat_map_hp, heat_map_cls,
                                                                     ...
                                                                     picked_locations, topK_prob_predicted_classes,
...
def cf_proposal_extraction(val_loader, heat_map_hp, heat_map_cls, imglist, dis_extracted_attributes, part_Locs, topK_prob_predicted_classes, remaining_mask_size_pool, maps):
    ...
    for i, (input, target, index) in enumerate(val_loader):
        ...
        for i_batch in range(index.shape[0]):
            ...
            for i_remain in range(np.size(remaining_mask_size_pool)):
                ...
                # find all unique part for true class

                for i_uq in range(np.size(topK_prob_predicted_classes, 1)):
                    if i_uq == 0:
                        cur_dis_attributes = dis_extracted_attributes[
                            topK_prob_predicted_classes[index[i_batch], i_uq], target[i_batch]]
