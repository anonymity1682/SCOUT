...
# top 10% dissimilarity are remain as ground truth
...
class Heatmap_hp:
    ...
    def __call__(self, input, refer_input, step):
        ...
        gradients = np.zeros((refer_input.shape[0], step+1, 512, 14, 14))
        ...
        for i_step in range(step + 1):
            ...
            gradients[:, i_step, :, :, :] = grads_val

        grads_val = np.mean(gradients, axis=1)
        ...
        cam_positive = (target - refer_features[-1].cpu().data.numpy()) * mask_positive
        cam_positive[cam_positive < 0.0] = 0.0
...
class Heatmap_cls:
    ...
    def __call__(self, input, PredictedClass, CounterfactualClass, refer_input=None, step=50):

        gradients = np.zeros((refer_input.shape[0], step + 1, 512, 14, 14, 2))
        for i_step in range(step + 1):
            cur_input = refer_input + (i_step / step) * (input - refer_input)
            features, output = self.extractor(cur_input.cuda())
            if i_step == 0:
            ...
            grads_val = grads_val[0].squeeze()
            grads_val = grads_val.cpu().data.numpy().squeeze()
            gradients[:, i_step, :, :, :, 1] = grads_val

        all_grads_val = np.mean(gradients, axis=1)
        target = features[-1]
        target = target.cpu().data.numpy()

        classifier_heatmaps = np.zeros((input.shape[0], np.size(target, 2), np.size(target, 2), 2))
        cam_positive = (target - refer_features[-1].cpu().data.numpy()[0, :]) * all_grads_val[:, :, :, :, 0].squeeze()
        cam_positive = np.sum(cam_positive, axis=1)
        classifier_heatmaps[:, :, :, 0] = cam_positive

        cam_positive = (target - refer_features[-1].cpu().data.numpy()[0, :]) * all_grads_val[:, :, :, :, 1].squeeze()
        cam_positive = np.sum(cam_positive, axis=1)
...
def cf_proposal_extraction(val_loader, heat_map_hp, heat_map_cls, imglist, dis_extracted_attributes, part_Locs, predicted_class, remaining_mask_size_pool):
    ...
    for i, (input, target, index) in enumerate(val_loader):
        ...
        refer_img = refer_img.repeat(index.shape[0], 1, 1, 1)
        refer_img = refer_img.cuda()


        easiness_heatmaps_set = heat_map_hp(input, refer_img, 50)
        easiness_mask_set = np.copy(easiness_heatmaps_set)
        easiness_mask_set[easiness_mask_set > 0] = 1
        classifier_heatmaps_set = heat_map_cls(input, predicted_class[index], target, refer_img, 50)
