...
parser.add_argument('--maps', default='as', help='explanation maps')
...
def main():
    ...
    # set the predicted from students are the second high poster probability of teachers
    topK_prob_predicted_classes, _ = smallest_indices_each_example(all_class_dis_teacher, 3)
    # all_correct_student = np.zeros(5794)
    all_predicted_student = topK_prob_predicted_classes[:, 1]
    all_gt_target_student = all_gt_target_teacher
    ...
    topK_prob_predicted_classes = topK_prob_predicted_classes[interested_idx, :]
    ...
    IOU = cf_proposal_extraction(val_hard_loader, heat_map_hp, heat_map_cls,
                                                                     ...
                                                                     picked_seg_list, topK_prob_predicted_classes,
...
def smallest_indices_each_example(all_response, topK):
    topK_maxIndex = np.zeros((np.size(all_response, 0), topK), dtype=np.int16)
    topK_maxValue = np.zeros((np.size(all_response, 0), topK))
    for i in range(np.size(topK_maxIndex, 0)):
        arr = all_response[i, :]
        topK_maxIndex[i, :] = np.argsort(arr)[:topK]
        topK_maxValue[i, :] = np.sort(arr)[:topK]
    return topK_maxIndex, topK_maxValue
...
class ModelOutputs_hp():
    ...
    def __call__(self, x):
        ...
        entropy = -1 * F.softmax(output, dim=1) * F.log_softmax(output, dim=1)
        normal_entropy = (entropy / torch.log(torch.tensor(output.shape[1], dtype=torch.float32).cuda()))
        normal_entropy = torch.sum(normal_entropy, dim=1)
        normal_easiness = 1 - normal_entropy
        return target_activations, normal_easiness
...
class Heatmap_hp:
    ...
    def __call__(self, input):
        ...
        mask_positive[mask_positive < 0.0] = 1e-100
...
def cf_proposal_extraction(val_loader, heat_map_hp, heat_map_cls, imglist, dis_extracted_attributes, seg_list, topK_prob_predicted_classes, remaining_mask_size_pool, maps):
    ...
    for i, (input, target, index) in enumerate(val_loader):
        ...
        classifier_heatmaps_set = heat_map_cls(input, topK_prob_predicted_classes[index, 0], target)
        ...
        for i_batch in range(index.shape[0]):
            ...
            for i_remain in range(np.size(remaining_mask_size_pool)):
                ...
                for i_uq in range(np.size(topK_prob_predicted_classes,1)):
                    if i_uq == 0:
                        cur_dis_attributes = dis_extracted_attributes[topK_prob_predicted_classes[index[i_batch], i_uq], target[i_batch]]
                        dis_attributes = cur_dis_attributes
                    else:
                        cur_dis_attributes = dis_extracted_attributes[topK_prob_predicted_classes[index[i_batch], i_uq], target[i_batch]]
                        dis_attributes = intersection(dis_attributes, cur_dis_attributes)
